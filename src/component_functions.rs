...
pub fn give_velocity(spell: &mut Spell, parameters: &[u64], should_execute: bool) -> Option<Vec<u64>> {
    ...
    match should_execute {
        false => Some(vec![f64::to_bits((spell.energy / 2.0) * ((parameter_one * parameter_one + parameter_two * parameter_two + parameter_three * parameter_three) as f64).sqrt() / APPLY_TO_SPELL_COEFFICIENT)]), // E_K = (1/2)mv^2
        true => {
